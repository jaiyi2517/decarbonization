...
export function UserGuideModal({ open, onClose }: UserGuideModalProps) {
  ...
  return (
    ...
    >
      <div
        className="flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-mist-200 bg-white shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-mist-200 bg-brand-50 px-6 py-4">
          <div className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-brand-600 text-white">
              <Info className="h-5 w-5" />
            </div>
            <div>
              <h2 className="text-base font-bold text-mist-800">使用說明 / 操作指南</h2>
              <p className="text-xs text-mist-400">Decarbonization Cost Calculator — User Guide</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Tab bar */}
        <div className="flex gap-1 border-b border-mist-200 bg-mist-50 px-4 py-2">
          {tabs.map((t) => {
            const Icon = t.icon;
            return (
              <button
                ...
                className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium transition ${
                  activeTab === t.id
                    ...
                    : 'text-mist-500 hover:bg-white/60 hover:text-mist-700'
                }`}
              >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {activeTab === 'purpose' && (
            <div className="space-y-4">
              <div className="rounded-lg bg-brand-50 p-4">
                <h3 className="mb-2 flex items-center gap-2 text-sm font-bold text-brand-700">
              ...
              <div className="space-y-2">
                <p className="text-sm font-semibold text-mist-700">核心解決問題：</p>
            ...
            <div className="space-y-4">
              <div className="rounded-lg border border-sky-100 bg-sky-50/50 p-4">
                ...
                <div className="space-y-3">
                  <div>
                    <p className="text-sm font-semibold text-mist-700">
                      預期外部碳價 / 碳稅 (p, €/tCO₂)
                    </p>
          ...
          {activeTab === 'chart' && (
            <div className="space-y-4">
              <div className="rounded-lg border border-mist-200 bg-mist-50/50 p-4">
                <h3 className="mb-3 flex items-center gap-2 text-sm font-bold text-mist-700">
                ...
                <div className="space-y-3 text-sm text-mist-600">
                  ...
                  <div className="rounded-lg bg-brand-50 p-3">
                    <p className="font-semibold text-brand-700">深色高亮長條 = 當前碳價下的黃金決策組合</p>
                    <p className="mt-1 text-sm text-brand-600">
                      圖中<strong>深主色長條</strong>代表在當前碳價下，使企業總財務負擔
                      Z(E,p) 最低的最優技術組合 (Optimal Technology Mix)；
                      <strong>淺主色長條</strong>則代表成本仍低於碳價、具經濟效益但非最優的組合。
                    </p>
                  </div>
                  <div className="rounded-lg bg-rust-50 p-3">
                    <p className="font-semibold text-rust-700">磚色虛線 = 當前外部碳價基準線</p>
                    <p className="mt-1 text-sm text-rust-600">
                      當虛線（碳價）<strong>高於</strong>某個階梯時，代表「自己花錢減碳比繳碳稅更便宜」
                      ，該階梯顯示為主色。當虛線<strong>低於</strong>階梯時，代表該技術組合
                      的邊際成本高於碳價，顯示為灰色。
                    </p>
                  </div>
                  <p>
                    <strong>互動方式</strong>：拖曳左側的「碳價滑桿」，虛線會即時上下移動，
          ...
          {activeTab === 'scenarios' && (
            <div className="space-y-3">
              <h3 className="flex items-center gap-2 text-sm font-bold text-mist-700">
              ...
              {[
                ...
                <div key={s.title} className={`rounded-lg border p-4 ${s.color}`}>
                  <p className="text-sm font-bold text-mist-700">{s.title}</p>
                  <p className="mt-1 text-sm leading-relaxed text-mist-600">{s.desc}</p>
                </div>
              ))}
            </div>
          )}
        ...
        {/* Footer */}
        <div className="border-t border-mist-200 bg-mist-50 px-6 py-3">
          <button
            onClick={onClose}
